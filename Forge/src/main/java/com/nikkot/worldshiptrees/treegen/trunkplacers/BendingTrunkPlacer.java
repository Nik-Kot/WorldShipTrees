...
public class BendingTrunkPlacer extends WorldShipTrunkPlacer {
   ...
   protected WorldShipTrunkPlacerType<?> type() {
      return WorldShipTrunkPlacerType.BENDING_TRUNK_PLACER;
   }

   public List<WorldShipFoliagePlacer.FoliageAttachment> placeTrunk(LevelSimulatedReader p_226079_, BiConsumer<BlockPos, BlockState> p_226080_, RandomSource p_226081_, int p_226082_, BlockPos p_226083_, WorldShipTreeConfiguration p_226084_) {
      ...
      for(int j = 0; j <= i; ++j) {
         ...
         if (WorldShipTreeFeature.validTreePos(p_226079_, blockpos$mutableblockpos)) {
            this.placeLog(p_226079_, p_226080_, p_226081_, blockpos$mutableblockpos, p_226084_);
         }

         if (j >= this.minHeightForLeaves) {
            list.add(new WorldShipFoliagePlacer.FoliageAttachment(blockpos$mutableblockpos.immutable(), 0, false));
         }
